...
unsigned int MyHammingCode::checkParityBit(unsigned int n)
{
  ...
  if(hammingWeight(checkBits) % 2 != checkBit(n, 0)){
    pos += 1;
  }
  checkBits = n & mask[1];

  if(hammingWeight(checkBits) % 2 != checkBit(n, 1)){
    pos += 2;
  }
  checkBits = n & mask[2];

  if(hammingWeight(checkBits) % 2 != checkBit(n, 3)){
    pos += 4;
  }
  checkBits = n & mask[3];

  if(hammingWeight(checkBits) % 2 != checkBit(n, 7)){
    pos += 8;
  }
  checkBits = n & mask[4];

  if(hammingWeight(checkBits) % 2 != checkBit(n, 15)){
    pos += 16;
  }
  return pos;
}
